"""
MCTS nSFT Training Script — SFT with Unlikelihood Loss on Negative Steps.

Extends SFT v2.1 with negative (wrong) steps:
  - Positive KEEP steps: standard NLL loss (same as SFT v2.1)
  - Negative WRONG steps: unlikelihood loss -log(1 - p(bad_token))
  - All steps share per-task difficulty weighting: (1-SR_t)^beta / T_t_total
  - No reference model needed, bounded loss, self-attenuating gradient

Usage:
    torchrun --nproc_per_node=8 scripts/train_mcts_nsft.py \
        --config configs/mcts_sft_v2.1_nsft.yaml
"""
...
class MCTSNSFTDataset(Dataset):
    """Negative-aware SFT dataset: positive KEEP steps + negative WRONG steps.

    Positive steps use NLL loss, negative steps use unlikelihood loss.
    All steps within a task share equal weight = (1-SR_t)^beta / T_t_total.
    """
    ...
    def __init__(
        ...
        n_pos = len(self.examples)
        self.is_negative = [False] * n_pos  # track positive vs negative
        ...
        print(f"  Positive KEEP steps: {n_pos}")

        # ---- Load negative WRONG steps ----
        print("Loading negative examples (audited WRONG steps)...")
        n_neg = 0
        with open(negatives_path) as f:
            for line in f:
                neg = json.loads(line.strip())
                if not neg.get("steps"):
                    continue  # skip entries without screenshots
                episode = {
                    "task_id": neg["task_id"],
                ...
                # train_all_steps=False → only the LAST step (the wrong action)
                ...
                if all_step_examples:
                    ...
                    self.examples.append(example)
                    self.is_negative.append(True)
                    task_keep_counts[neg["task_id"]] += 1
                    n_neg += 1

        self.n_pos = n_pos
        self.n_neg = n_neg
        self.num_tasks = len(task_keep_counts)
        print(f"  Negative WRONG steps: {n_neg}")
        print(f"  Total: {len(self.examples)} (pos={n_pos}, neg={n_neg})")

        # ---- Compute per-step weights (unified: pos + neg share task weight) ----
        raw_weights = []
        for ex in self.examples:
            tid = ex["task_id"]
            sr = task_sr.get(tid, 0.0)
            T_t = task_keep_counts[tid]  # total steps (pos+neg) for this task
            w = ((1 - sr) ** beta) / T_t
    ...
    def __getitem__(self, index):
        ...
        result = {
            ...
            "is_negative": torch.tensor(1.0 if self.is_negative[index] else 0.0, dtype=torch.float32),
        }
        return result
...
def _parse_args():
    ...
    defaults = {
        ...
        "output_dir": "checkpoints/mcts_nsft",
        "num_epochs": 1,
