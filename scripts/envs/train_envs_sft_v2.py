"""
ENVS SFT v2 Training Script — Per-Step Deduplicated, Power-Scaled Weighting.

Trains on successful ENVS tree trajectories with:
  1. Per-step deduplication: shared prefix steps trained exactly once
  // NOTE: depreciated 2. Step-level KEEP/REMOVE masks (agent-audited)
...
from verl.envs.tree_io import load_envs_tree, reconstruct_steps, _get_ancestor_chain
...
class ENVSSFTDatasetV2(Dataset):
    """Per-step deduplicated SFT dataset from ENVS trees.
    ...
    def __init__(
        ...
        max_leaves_per_task: Optional[int] = None,
        leaf_sample_seed: int = 0,
    ):
        ...
        n_tasks_capped = 0
        cap_examples = []  # (task_id, n_available, n_kept) for a few capped tasks (for logging)

        for task_id, tree_files in task_index.items():
            # Load this task's trees once and keep them for Pass 2.
            loaded_trees = []  # list of (tree_filename, tree_data)
            for tree_filename in tree_files:
                tree_path = os.path.join(tree_dir, tree_filename)
                real_path = os.path.realpath(tree_path)
                if not os.path.exists(real_path):
                    continue
                tree_data = load_envs_tree(real_path)
                loaded_trees.append((tree_filename, tree_data))

            # Pass 1: enumerate successful leaves across this task's trees.
            # Preserve the same (tree_files × nodes) iteration order as the
            # original single-pass code so the no-cap path is a no-op.
            task_entries = []  # list of (tree_filename, leaf_id)
            for tree_filename, tree_data in loaded_trees:
                for leaf_id, leaf in tree_data["nodes"].items():
                    if leaf.get("eval_score") and leaf["eval_score"] > 0:
                        task_entries.append((tree_filename, leaf_id))

            n_available = len(task_entries)

            # Apply per-task leaf cap (deterministic shuffle + truncate).
            # Use zlib.crc32 (cross-process stable) instead of Python's
            # per-process-randomized hash() — otherwise each rank samples a
            # different leaf subset → different dataset sizes → FSDP rank-skew
            # deadlock at epoch end.
            if max_leaves_per_task is not None and n_available > max_leaves_per_task:
                entries_sorted = sorted(task_entries)
                task_crc = zlib.crc32(task_id.encode("utf-8"))
                rng = random.Random(
                    (leaf_sample_seed * 2654435761 + task_crc) & 0xFFFFFFFF
                )
                rng.shuffle(entries_sorted)
                task_entries = sorted(entries_sorted[:max_leaves_per_task])
                n_tasks_capped += 1
                if len(cap_examples) < 5:
                    cap_examples.append((task_id, n_available, len(task_entries)))

            # Pass 2: process selected leaves (identical logic to pre-refactor).
            tree_by_name = {fn: td for fn, td in loaded_trees}
            for tree_filename, leaf_id in task_entries:
                tree_data = tree_by_name[tree_filename]
                ...
                for k, (node_id, node_step_idx) in enumerate(step_sources):
                    ...
                    # NOTE (deprecated): the KEEP/REMOVE step-mask tag is no longer
                    # used. Current mask files contain no REMOVE (0) entries, so every
                    # step defaults to KEEP and this block is effectively a no-op. Kept
                    # only for backwards compatibility with old step_masks_v2.json files.
                    # Check mask
        ...
        if max_leaves_per_task is not None:
            print(f"  Per-task leaf cap: {max_leaves_per_task} (seed={leaf_sample_seed})")
            print(f"  Tasks capped: {n_tasks_capped}/{len(task_index)}")
            for tid, n_avail, n_kept in cap_examples:
                print(f"    {tid[:12]}: {n_avail} -> {n_kept}")
...
_v1_spec = importlib.util.spec_from_file_location(
    "train_envs_sft",
    # v1 base module lives in scripts/ (one level up from scripts/envs/)
    os.path.join(os.path.dirname(__file__), "..", "train_envs_sft.py"),
)
_v1_mod = importlib.util.module_from_spec(_v1_spec)
_v1_spec.loader.exec_module(_v1_mod)

envs_collate_fn = _v1_mod.envs_collate_fn
ENVSTrainer = _v1_mod.ENVSTrainer
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="ENVS SFT v2 — Per-Step Deduplicated, Power-Scaled Weighting")
    ...
    parser.add_argument("--max_leaves_per_task", type=int, default=None,
                        help="Optional per-task hardcap on successful leaves loaded for training "
                             "(only easier tasks with many leaves get capped; hard tasks kept intact). "
                             "Default: None (no cap, full v2.1 behavior).")
    parser.add_argument("--leaf_sample_seed", type=int, default=None,
                        help="Seed for deterministic leaf subsampling when max_leaves_per_task is set. "
                             "Default: 0.")
    ...
    defaults = {
        ...
        "output_dir": "checkpoints/envs_sft_v2",
        "num_epochs": 1,
...
def main():
    ...
    dataset = ENVSSFTDatasetV2(
        ...
        max_leaves_per_task=args.max_leaves_per_task,
        leaf_sample_seed=args.leaf_sample_seed,
    )
    ...
    print("Starting ENVS SFT v2 training...")
