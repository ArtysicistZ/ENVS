#!/usr/bin/env python3
"""
Production ENVS trajectory collection.
...
logger = logging.getLogger("envs_collection")
...
def load_doable_task_ids():
    ...
    # Source 1: ENVS v1 successful trajectories (84 tasks)
    envs_path = os.path.join(PROJ_ROOT, "checkpoints/envs_trajectories/combined/envs_success.jsonl")
...
def create_workers(urls, counts, ray_module):
    from verl.envs.env_client import ENVSEnvClient
    RemoteClient = ray_module.remote(num_cpus=0, num_gpus=0)(ENVSEnvClient)
    workers = []
    worker_idx = 0
    for url, count in zip(urls, counts):
        for slot in range(count):
            ...
            worker_idx += 1
    total = sum(counts)
    logger.info("Created %d ENVSEnvClient actors across %d servers", total, len(urls))
    for url, count in zip(urls, counts):
...
def main():
    ...
    # Create VLLMPool
    from verl.envs.vllm_pool import VLLMPool
    vllm_pool = VLLMPool(
        n_gpus=cfg["tp"],
    ...
    # Create ENVSConfig from yaml
    from verl.envs.config import ENVSConfig
    from verl.envs.orchestrator import ENVSOrchestrator
    ...
    if config.enable_noise:
        logger.info("Noisy ENVS enabled: branch_prob=%.2f, min_fire_step=%d, min_buffer=%d",
                     config.noise_branch_probability, config.noise_min_fire_step,
                     config.noise_min_task_buffer)

    orchestrator = ENVSOrchestrator(config, vllm_pool, processor, tokenizer,
    ...
    for task_idx, task_config in enumerate(all_tasks):
        ...
        result = {
            ...
            "envs_time_sec": round(envs_time, 1),
            "reset_time_sec": round(reset_time, 1),
        }
        results.append(result)
        logger.info("  VMs: %d, Success: %d/%d, Time: %.1fs",
                    n_activated, n_success, len(trajectories), envs_time)
        ...
        if cfg.get("save_full_tree", False):
            from verl.envs.tree_io import save_envs_tree
            trees_dir = os.path.join(output_dir, "trees")
            os.makedirs(trees_dir, exist_ok=True)
            tree_path = os.path.join(trees_dir, f"{tid}.json")
            save_envs_tree(tree, task_config, tree_path, limit_images=cfg["limit_images"])
        ...
        if successful:
            success_path = os.path.join(output_dir, "envs_success.jsonl")
