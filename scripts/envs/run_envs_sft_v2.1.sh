...
# Resolve repo root from this script's location (scripts/envs/ -> repo root) so
# the launcher is not tied to any one machine's checkout path.
...
echo "=============================================="
echo "  ENVS SFT v2.1 — Training + n8 Eval"
echo "  Config: ${CONFIG}"
...
# ==============================
echo ""
echo "[Step 1/3] Training ENVS SFT v2.1..."
echo "  lr=2e-6, beta=0.5, max_step_ratio=3, max_grad_norm=1.0"
echo "  effective_batch=32 (1 x 4 accum x 8 GPUs)"
echo ""

sudo -E ${TORCHRUN} --nproc_per_node=8 scripts/envs/train_envs_sft_v2.py \
    --config "${CONFIG}" \
    --run_name "envs_sft_v2.1_beta05_2e-6"
...
sudo pkill -f "torchrun.*train_envs_sft_v2" 2>/dev/null || true
...
sudo -E ${PYTHON} -m verl.trainer.main \
    ...
    trainer.experiment_name="envs_sft_v2.1_eval_n8" \
