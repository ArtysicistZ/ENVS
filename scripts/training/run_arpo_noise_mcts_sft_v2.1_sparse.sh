#!/usr/bin/env bash
# ARPO + v4 sparse_mcts Noise from MCTS SFT v2.1 (full) checkpoint.
#
# Topology:
...
#
# Usage:
#   bash scripts/training/run_arpo_noise_mcts_sft_v2.1_sparse.sh
#   bash scripts/training/run_arpo_noise_mcts_sft_v2.1_sparse.sh trainer.total_episodes=5
...
CONFIG="configs/arpo_8gpu_noise_mcts_sft_v2.1_sparse.yaml"
...
LOG="${ROOT_DIR}/checkpoints/arpo_mcts_sft_v2.1_sparse_noise_${TS}.log"
mkdir -p "$(dirname "$LOG")"

echo ""
echo "=== Launching ARPO + v4 sparse_mcts noise (MCTS SFT v2.1 full) ==="
echo "  config: $CONFIG"
