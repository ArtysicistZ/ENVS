#!/usr/bin/env bash
# ARPO + v4 sparse_mcts Noise from MCTS SFT v2.1 base_30pct checkpoint.
#
# Topology:
...
#
# Usage:
#   bash scripts/training/run_arpo_noise_mcts_sft_v2.1_base_30pct.sh
#   bash scripts/training/run_arpo_noise_mcts_sft_v2.1_base_30pct.sh trainer.total_episodes=5
...
CONFIG="configs/arpo_8gpu_noise_mcts_sft_v2.1_base_30pct.yaml"
...
LOG="${ROOT_DIR}/checkpoints/arpo_mcts_sft_v2.1_base_30pct_sparse_noise_${TS}.log"
mkdir -p "$(dirname "$LOG")"

echo ""
echo "=== Launching ARPO + v4 sparse_mcts noise (MCTS SFT v2.1 base_30pct) ==="
echo "  config: $CONFIG"
echo "  model:  checkpoints/mcts_sft_v2.1_base_30pct/beta05_5.6e-6/epoch_1"
echo "  save_freq: 35 steps"
echo "  fire counts: SR<0.15→0, 0.15-0.60→1, >0.60→2"
