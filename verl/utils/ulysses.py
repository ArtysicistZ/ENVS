...
def ulysses_pad_and_slice_inputs(
    ...
    Pad and slice position_ids to be divisible by sp_size.

    Both input_ids_rmpad and position_ids_rmpad are sliced along the sequence dimension so
    each SP rank sees matching local token and RoPE lengths.

    The is the utility of pre-forward for ulysses sequence parallelism

    Args:
        input_ids_rmpad: shape of [bsz, seqlen]
        position_ids_rmpad: shape of [..., seqlen] sharing the same last-dim sequence length
