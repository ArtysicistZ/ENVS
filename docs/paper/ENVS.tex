...
\begin{tikzpicture}
  \begin{axis}[
      width=0.6\linewidth,
      height=5.5cm,
      xlabel={task success rate $p$},
      ylabel={gradient mass (normalized)},
      xmin=0, xmax=1,
      ymin=0, ymax=1.1,
      ...
      % --- 独立的图例设置 ---
     legend style={
          at={(0.5, -0.3)},      % 0.5代表水平居中，-0.3代表放在X轴下方一段距离，避开xlabel
          anchor=north,          % 以图例顶端为锚点挂在那里
          legend columns=-1,     % 让两项图例横向并排显示，不占垂直空间
          draw=none, 
          fill=none, 
          font=\footnotesize
      }
  ...
  \addlegendentry{\textsc{ENVS}, $\beta{=}0.5$} % 注册到图例

  % GRPO curve: 1 - x^8 - (1-x)^8
  \addplot[
      domain=0:1,
      samples=150,
      thick,
  ...
  \addlegendentry{GRPO, $G{=}8$} % 注册到图例

  \end{axis}
\end{tikzpicture}
\caption{Per-task gradient mass against task success rate. The GRPO
curve (red, dashed) has gradient vanish at both $p = 0$ and $p = 1$. The
\textsc{ENVS} curve (blue) monotonically decreases, so gradient mass shifts towards harder tasks rather than collapsing at the saturated endpoint.}
\label{fig:rl:gradient-curves}
\end{figure}
