...
% \ifdefined\pdfoutput\pdfoutput=1\fi   % force pdfLaTeX on arXiv; no-op under XeLaTeX/LuaLaTeX
...
\captionsetup[figure]{skip=4pt}   % tighten image -> caption gap for all figures (default 10pt)
...
\title{\vspace{-2.0em}\Large\bfseries ENVS: Environment-Native Verified Search for Long-Horizon GUI Agents
}

\author{
Yincheng Zhou$^{1,*}$ \quad Athena Zhuoming Zhong$^{1,*}$ \quad Shijie Zhang$^{2,*}$\\
Kevin Zhang$^{2}$ \quad Teresa Xiaotao Shang$^{1}$ \quad Shanghang Zhang$^{2,\dagger}$\\[0.5em]
...
\begin{itemize}[leftmargin=*, itemsep=0.15em, topsep=0.15em, parsep=0pt, partopsep=0pt]
    \item \textbf{Clean performance:} \textsc{ENVS} reaches 30.3 pass@8 on \textsc{OSWorld}, improving UI-TARS-1.5 from 22.7 and outperforming completed ARPO-style baselines at 26.7.
    ...
    % \item \textbf{Compute efficiency:} \textsc{ENVS} reduces matched ARPO compute from 184--192 to 138--153 GPU-hours, a 20--25\% reduction.
...
\end{aligned}
\]
$(1-\mathrm{SR}_i)^\beta$ shifts gradient mass to hard tasks; $1/T_i$ normalizes trajectory; $w_{\max}$ caps rare-success outliers. Smaller $\beta$ ensures uniform weighting; larger $\beta$ emphasizes hard tasks more aggressively.
...
\textsc{OSWorld-Noisy} draws from a library of $153$ runtime noise
...
exclusively for evaluation; the remaining $129$ are available for
...
ENVS uses 138--153 total GPU-hours, compared with 184--192 for matched
ARPO baselines (Table~\ref{tab:compute}).
The saving comes from decoupling collection from optimization: ENVS
collects verified trajectories once and trains only successful trajectories with one-epoch SFT,
whereas ARPO collects fresh on-policy rollouts inside each training run, updating policy with all trajectories.
...
\begin{table}[H]
  ...
  column dashed). Bold marks the column minimum within each block.}
  \label{tab:compute}
  \centering
  \small
  \begin{tabular}{llccc}
    ...
    UI-TARS-1.5-7B  & + ENVS           & $31$        & $107$         & $\mathbf{138}$              \\
    \midrule
    \multicolumn{5}{l}{\textit{Noisy-trained}} \\
    UI-TARS-1.5-7B  & + ARPO           & $192$       & ---           & $192$                       \\
    UI-TARS-1.5-7B  & + ENVS           & $32$        & $121$         & $\mathbf{153}$              \\
    \bottomrule
  \end{tabular}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\linewidth]{data-efficiency.png}
...
using less compute. The reduced-data result shows that a
...
\textsc{OSWorld-Noisy} contains 153 runtime noise generators. A held-out subset of 24 generators is used only for evaluation. Table~\ref{tab:noise-taxonomy-full} gives the expanded taxonomy used by the implementation. Counts refer to generator entries; a single generator can produce many concrete events through internal randomization.
...
Pass@8 numbers reported in Section~\ref{sec:experiments} therefore include both trained-on
...
regression of ARPO-noisy, $54/86$ against $57/86$ for the base policy in
...
branching retains the $k=3$ highest-agreement action clusters (one on the parent VM and up to two on child VMs)
